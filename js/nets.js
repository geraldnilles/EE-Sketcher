/* =====================================================================
   ...
   All split/merge/junction logic runs as stateless global passes across
   the entire DOM rather than targeted, brittle, per-operation logic.
   ===================================================================== */
(function (global) {
  'use strict';
   ...
   * Snaps to grid, validates, creates the line element, then runs the
   * full stateless topology pass.  Returns the new <line> or null.
   * 
   * There is no per-operation "extend" or "bridge" logic here.  Those
   * behaviours emerge naturally from the stateless refreshNetTopology()
   * engine which runs splitAllLines() → mergeLines() → recomputeJunctions()
   * across every line on the canvas.
   */
  ...
  /**
   * splitAllLines()
   * Statelessly scans every line against all line endpoint coordinates.
   * If any endpoint lies strictly in the interior of any line segment,
   * that segment is split in two.  Loops until no more splits occur.
   */
  ...
  /**
   * mergeLines()
   * Stateless pass: scan every pair of collinear lines on the same
   * axis.  If their intervals overlap (positive-length intersection)
   * they are always merged into their envelope.  If their intervals
   * merely touch at a single point (endpoint-to-endpoint) they are
   * only merged when that shared coordinate has exactly two line
   * endpoints — i.e. it is a plain butt-join, not a T-junction.
   *
   * Loops until no more merges are possible.
   */
  function mergeLines() {
    const layer = document.getElementById('nets-layer');
    if (!layer) return 0;
