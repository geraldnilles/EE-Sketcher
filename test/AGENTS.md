...
| `test_components.py` | Component creation via toolbar buttons and keyboard shortcuts (A for generic). Covers: generic blocks, GND, VDD, resistor, capacitor, inductor, diode. Verifies the component lands in `#components-layer`, gets `.is-selected`, has a `data-id`, and (for generics) has `.pin-left` / `.pin-right` text labels. Also verifies `<use>` element references correct SVG def for all passives. |
...
| `test_inspector.py` | Inspector sidebar panel rendering. Verifies per-type panels (generic shows pin labels/width/rows/labels and secondary checkbox; passive shows label/rotation; VDD shows label; GND shows read-only info; line shows coords). Tests that editing fields (fill + Enter) updates the live component, and that the secondary checkbox toggles `data-secondary`. |
