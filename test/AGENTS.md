...
| `test_components.py` | Component creation via toolbar buttons and keyboard shortcuts (A for generic). Covers: generic blocks, GND, VDD, resistor, capacitor, inductor, diode, and comment components. Verifies the component lands in `#components-layer`, gets `.is-selected`, has a `data-id`, and (for generics) has `.pin-left` / `.pin-right` text labels. Also verifies `<use>` element references correct SVG def for all passives and that comment components have `rect.comment-body` and initial text line. |
...
| `test_inspector.py` | Inspector sidebar panel rendering. Verifies per-type panels (generic shows pin labels/width/rows/labels and secondary checkbox; comment shows "Text Comment" header, line text inputs, +/- line buttons, position info, and delete button; passive shows label/rotation; VDD shows label; GND shows read-only info; line shows coords). Tests editing fields update the live component, secondary checkbox toggles `data-secondary`, comment line add/remove, and comment auto-delete on blank blur. |
| `test_deletion.py` | Element deletion. Covers: sidebar "Delete Component" button, "Delete Comment" button, X key, Backspace key, Delete key. Verifies element removal from DOM, cleared selection, and inspector reset to "Nothing selected". Also tests deletion of net lines and comment components. |
...
| `test_portal.py` | Data Portal export/import. Verifies that "Export Schema" populates the textarea with SVG, that exported text contains expected elements (including comment components), and that "Import Schema" restores components with comment text intact. Also tests import of empty/malformed text produces an error status. |
| `test_integration.py` | Full end-to-end workflow. Adds multiple components (including comments), labels them, draws nets between them, drags a component, exports, resets, imports, and verifies round-trip fidelity including comment text. |
